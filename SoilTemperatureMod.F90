...
contains
  ...
  !-----------------------------------------------------------------------
  subroutine Phasechange_beta (bounds, num_nolakec, filter_nolakec, dhsdT, fn_mod, &
       soilstate_vars, waterstate_vars, waterflux_vars, energyflux_vars, temperature_vars)
    !
    ! !DESCRIPTION:
    ...
    use clm_varpar       , only : nlevsno, nlevgrnd,nlevurb
    use clm_varctl       , only : iulog
    use clm_varcon       , only : denh2o, denice, tfrz, hfus, grav
    ...
    integer                , intent(in)    :: filter_nolakec(:)                    ! column filter for non-lake points
    real(r8)               , intent(in)    :: dhsdT ( bounds%begc: )               ! temperature derivative of "hs" [col]
	real(r8)               , intent(in)    :: fn_mod(bounds%begc:bounds%endc,-nlevsno+1:nlevgrnd)
    ...
    associate(                                                        & 
      ...
      dtime = get_step_size()
	  nstep = get_nstep()
	  !write(iulog,"(A30,I20)") 'Phasechange_beta nstep:', nstep
         ...
         do fc = 1,num_nolakec
            ...
            if ((col_pp%itype(c) /= icol_sunwall .and. col_pp%itype(c) /= icol_shadewall &
               ...
               if (h2osoi_ice(c,j) > 0. .AND. t_soisno(c,j) > tfrz) then
                  imelt(c,j) = 1					!thawing 
                  !tinc(c,j) = tfrz - t_soisno(c,j) 	!negative
                  !t_soisno(c,j) = tfrz
               ...
               if (h2osoi_liq(c,j) > supercool(c,j) .AND. t_soisno(c,j) < supertemp(c,j)) then
                  imelt(c,j) = 2					!freezing
                  !tinc(c,j) = tfrz - t_soisno(c,j) 	!positive
                  !t_soisno(c,j) = tfrz
            ...
            if ((col_pp%itype(c) /= icol_sunwall .and. col_pp%itype(c) /= icol_shadewall &
               ...
               if (j >= snl(c)+1) then
                  ...
                  if (imelt(c,j) > 0) then
                     ...
                     !==================================================================
                     if (j == snl(c)+1) then ! top snow layer   
                     ...
                     else if (j == 1) then	!top soil layer
                  ...
                  if (imelt(c,j) > 0 .and. abs(hm(c,j)) > 0._r8) then
                     ...
                     if (abs(heatr) > 0._r8) then
                        ...
                        else if (j == 1) then	!top layer

                           t_soisno(c,j) = t_soisno(c,j) + fact(c,j)*heatr &
                                /(1._r8-(1.0_r8 - frac_sno_eff(c) - frac_h2osfc(c))*fact(c,j)*dhsdT(c))
                        else
						
                           if(j > 0) then	!Soil layers, not including the 1st layer
                     ...
                     if (j >= 1) then 
                        xmf(c) = xmf(c) + hfus*(wice0(c,j)-h2osoi_ice(c,j))/dtime + eng_ch(c,j)	!for soil energy balance
                     else
