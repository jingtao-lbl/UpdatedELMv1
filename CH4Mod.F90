...
contains
      ...
      do j=1,nlevsoi
         do fc = 1, num_methc
            ...
            if ( (.not. lake) .and. sat == 1 .and. finundated_lag(c) < finundated(c)) then
               ...
               !write(iulog,*) 'Redox factor saturated:',jwt(c),finundated_lag(c),finundated(c)
               ...
               !write(iulog,*) 'Redox factor unsaturated:',jwt(c),finundated_lag(c),finundated(c)
            end if
            ...
            ! in CH4varcon:
				!anoxicmicrosites = .false. ! Use Arah & Stephen 1998 expression to allow production above the water table
      ...
      if (.not. lake) then
         do j=1,nlevsoi
            do fp = 1, num_methp
               ...
               !Jing Tao - allow aerenchyma diffusion even when T <= tfrz
               if (j > jwt(c) .and. veg_pp%itype(p) /= noveg) then
                  ...
                  if (usefrootc) then
					 ...
					 m_tiller = anpp * nppratio * max(elai(p), 0.01_r8)	!Jing Tao modified
                  end if
  ...
  subroutine ch4_tran (bounds, &
	...
	!Jing Tao: 
	real(r8) :: scale_factor_gasdiff_snow
	real(r8) :: scale_factor_liqdiff_snow
	real(r8) :: scale_factor_liqdiff_frzsoil
    ...
    associate(                                                 & 
	  ...
	  ! Jing Tao:
	  scale_factor_gasdiff_snow = scale_factor_gasdiff*100
	  scale_factor_liqdiff_snow = scale_factor_liqdiff*100
	  scale_factor_liqdiff_frzsoil = 0.5
         ...
         do j = -nlevsno + 1,0
            do fc = 1, num_methc
               ...
               if (j == 0) then ! final loop
                  ...
                  !spec_grnd_cond(c,s) = 1._r8/(1._r8/grnd_ch4_cond(c) + snowres(c))
                  !spec_grnd_cond(c,s) = grnd_ch4_cond(c)
				  
                  !write(iulog,"(A10,3(I4),4(1X,F20.6))") 'TJ-Check1:', c, s, sat, grnd_ch4_cond(c), snowres(c), pondres, spec_grnd_cond(c,s)
         ...
         do j = 1,nlevsoi
            do fc = 1, num_methc
               ...
               diffus(c,j) = max(diffus(c,j), smallnumber) ! Prevent overflow
                ...
                !write(iulog,"(A10,5(I4),4(1X,F20.6))") 'TJ-Check:', c, s, sat, jwt(c), j, diffus (c,j), t_soisno_c,h2osoi_liq(c,j), (h2osoi_liq(c,j)/denh2o+smallnumber)/(h2osoi_liq(c,j)/denh2o+h2osoi_ice(c,j)/denice+smallnumber)
