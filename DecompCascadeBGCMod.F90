  ...
  subroutine decomp_rate_constants_bgc(bounds, num_soilc, filter_soilc, &
    ...
    associate(                                             &
     ...
     ! The following code implements the acceleration part of the AD spinup algorithm
      ...
      if ( nlevdecomp .eq. 1 ) then
         ...
         if ( .not. use_century_tfunc ) then !use_century_tfunc = .false.
            ...
            do j = 1, nlevdecomp
               do fc = 1,num_soilc
				  ...
				  if (t_soisno(c,j) >= (SHR_CONST_TKFRZ -30._r8)) then
				  
				  !Century 1, Burke et al. [2003]
				  ...
				  !Daycent1, Kelly et al. [2000]
				  !t_scalar(c,j) = 0.08*exp(0.095*(t_soisno(c,j)-SHR_CONST_TKFRZ))
				  
				  !Daycent2, Parton et al. [2001[and Grosso et al. [2005]
				  !t_scalar(c,j) = max(0.56+(1.46*atan(SHR_CONST_PI*0.0309*(t_soisno(c,j)-SHR_CONST_TKFRZ-15.7)))/SHR_CONST_PI, 0._r8)
				  
				  !Q10=2.0(Tref=10)
				  !t_scalar(c,j) = 2.0**((t_soisno(c,j)-SHR_CONST_TKFRZ-10._r8)/10._r8)
				  
				  !Q10=1.4(Tref=10)
				  !t_scalar(c,j) = 1.4**((t_soisno(c,j)-SHR_CONST_TKFRZ-10._r8)/10._r8)
				  
				  !Lloyd and Taylor, Lloyd and Taylor [1994]
				  !t_scalar(c,j) = max(exp(308.56*(1._r8/56.02-1._r8/((t_soisno(c,j)-SHR_CONST_TKFRZ+273._r8)-227.13))), 0._r8)
				  
				  !Kirschbaum,Kirschbaum [1995]
				  ...
				  !Standcarb, Harmon and Domingo [2001]
				  !t_scalar(c,j) = exp(-1._r8*((t_soisno(c,j)-SHR_CONST_TKFRZ)/(45._r8+4._r8))**15._r8)*2.0**(((t_soisno(c,j)-SHR_CONST_TKFRZ)-10._r8)/10._r8)
				  
				  !TWfun27:Q10=1.3(Tref=25)
				  !t_scalar(c,j) = 1.3**((t_soisno(c,j)-SHR_CONST_TKFRZ-25._r8)/10._r8)
				  
				  !TWfun28:Q10=1.4(Tref=25)
				  !t_scalar(c,j) = 1.4**((t_soisno(c,j)-SHR_CONST_TKFRZ-25._r8)/10._r8)
				  
				  !TWfun29:Q10=1.5(Tref=25), Same as Default
				  !t_scalar(c,j) = 1.5**((t_soisno(c,j)-SHR_CONST_TKFRZ-25._r8)/10._r8)
				  
				  !TWfun30:Q10=2(Tref=25)
				  t_scalar(c,j) = 2.0**((t_soisno(c,j)-SHR_CONST_TKFRZ-25._r8)/10._r8)
				  
				  !TWfun31:Q10=1.5(Tref=35)
				  !t_scalar(c,j) = 1.5**((t_soisno(c,j)-SHR_CONST_TKFRZ-35._r8)/10._r8)
				  
				  !TWfun32:Q10=2(Tref=35)
				  !t_scalar(c,j) = 2.0**((t_soisno(c,j)-SHR_CONST_TKFRZ-35._r8)/10._r8)
         ...
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
			   ...
			   !Jing Tao - Modified w_scalar, Scheme 2
			   if (vsm_pec .gt. 0.65) then 
			      w_scalar(c,j) = w_scalar(c,j)*((1-vsm_pec)/(1-0.65))**0.75
			   end if
			   
			   if (vsm_pec .gt. 0.1) then
			   	  w_scalar(c,j) = max(0.2_r8,w_scalar(c,j))
			   else
			   	  w_scalar(c,j) = max(0.1_r8,w_scalar(c,j))
			   end if
			   
			   
			   !Century, Parton et al. [2001] and Adair et al. [2008]
			   !w_scalar(c,j) = min(1.0_r8, 1/(1+30*exp(-8.5*vsm_pec)))
			   
			   !Candy, Bauer et al. [2008]
			   !if (vsm_pec .gt. 0.5) then
			   ! w_scalar(c,j) = 1.0_r8
			   !else
			   ! w_scalar(c,j) = 4*vsm_pec*(1-vsm_pec)
			   !end if
			   
			   !Moyano, Moyano et al. [2013]
			   !w_scalar(c,j) = 3.11*vsm_pec-2.42*vsm_pec*vsm_pec
			   
			   !Demeter, Foley [2011]
			   !w_scalar(c,j) = 0.25+0.75*vsm_pec
			   
			   !Daycent for coarse-textured soils, Kelly et al. [2000]
			   !w_scalar(c,j) = ((vsm_pec-1.70)/(0.55-1.70))**(3.22*((1.70-0.55)/(0.55+0.007)))*((vsm_pec+0.007)/(0.55+0.007))**3.22
			   
			   !Myers et al. [1982]
			   !w_scalar(c,j) = 2.2*vsm_pec+(1-2.2)*vsm_pec**2	!Myers2
			   !w_scalar(c,j) = 1.2*vsm_pec+(1-1.2)*vsm_pec**2	!Myers1
			   
			   !Gompertz, Janssens et al. [2003]
			   !w_scalar(c,j) = exp(-1*exp(0.824-0.308*vsm_pec*100))
			   
			   !Standcarb, Harmon and Domingo [2001]
			   !w_scalar(c,j) = ((1-exp(-1._r8*(3._r8/15._r8)*(vsm_pec*100+0._r8)))**5._r8)*(exp(-1._r8*(vsm_pec*100/(90._r8+4._r8))**15._r8))
			   
			   !Skopp et al. [1990]
			   !w_scalar(c,j) = min(2*vsm_pec**1.3,2*(1-vsm_pec)**0.8)
		...
		!do fc = 1,num_soilc
		!c = filter_soilc(fc)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'w_scalar:', c, (w_scalar(c,j), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'liq:', c, (h2osoi_liq(c,j), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'porosity:', c, (watsat(c,j), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'maxpsi:', c, (sucsat(c,j) * (-9.8e-6_r8), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'psi:', c, (soilpsi(c,j), j = 1, nlevdecomp)
	    !end do
      ...
      if (use_vertsoilc) then
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
			   ...
			   !SA2
			   !decomp_k(c,j,i_litr1) = k_l1    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j)
               !decomp_k(c,j,i_litr2) = k_l2_l3 * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j)
               !decomp_k(c,j,i_litr3) = k_l2_l3 * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j)
               !decomp_k(c,j,i_soil1) = k_s1    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j)
               !decomp_k(c,j,i_soil2) = k_s2    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j)
               !decomp_k(c,j,i_soil3) = k_s3    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j)
			   
			   !decomp_k(c,j,i_litr1) = k_l1    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_litr2) = k_l2_l3 * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_litr3) = k_l2_l3 * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_soil1) = k_s1    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_soil2) = k_s2    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_soil3) = k_s3    * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
			   
			   !SA3
			   !decomp_k(c,j,i_litr1) = k_l1    * (t_scalar(c,j) + w_scalar(c,j))/2.0
               !decomp_k(c,j,i_litr2) = k_l2_l3 * (t_scalar(c,j) + w_scalar(c,j))/2.0
               !decomp_k(c,j,i_litr3) = k_l2_l3 * (t_scalar(c,j) + w_scalar(c,j))/2.0
               !decomp_k(c,j,i_soil1) = k_s1    * (t_scalar(c,j) + w_scalar(c,j))/2.0
               !decomp_k(c,j,i_soil2) = k_s2    * (t_scalar(c,j) + w_scalar(c,j))/2.0
               !decomp_k(c,j,i_soil3) = k_s3    * (t_scalar(c,j) + w_scalar(c,j))/2.0
			   
			   !SA4
			   decomp_k(c,j,i_litr1) = k_l1    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j)
               decomp_k(c,j,i_litr2) = k_l2_l3 * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j)
               decomp_k(c,j,i_litr3) = k_l2_l3 * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j)
               decomp_k(c,j,i_soil1) = k_s1    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j)
               decomp_k(c,j,i_soil2) = k_s2    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j)
               decomp_k(c,j,i_soil3) = k_s3    * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j)
			   
			   !SA5
			   !decomp_k(c,j,i_litr1) = k_l1    * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_litr2) = k_l2_l3 * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_litr3) = k_l2_l3 * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_soil1) = k_s1    * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_soil2) = k_s2    * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
               !decomp_k(c,j,i_soil3) = k_s3    * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
         ...
         if (.not. use_fates) then
            do j = 1,nlevdecomp
               do fc = 1,num_soilc
				  ...
				  !SA2
				  !decomp_k(c,j,i_cwd)   = k_frag  * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) 
				  !decomp_k(c,j,i_cwd)   = k_frag  * min(t_scalar(c,j), w_scalar(c,j))* depth_scalar(c,j) * o_scalar(c,j)
				  
				  !SA3
				  !decomp_k(c,j,i_cwd)   = k_frag  * (t_scalar(c,j) + w_scalar(c,j))/2.0
				  
				  !SA4
				  decomp_k(c,j,i_cwd)   = k_frag  * t_scalar(c,j) * w_scalar(c,j) * depth_scalar(c,j) 
				  
				  !SA5
				  !decomp_k(c,j,i_cwd)   = k_frag  * t_scalar(c,j) * max(w_scalar(c,j), 0.2) * depth_scalar(c,j) * o_scalar(c,j)
	  ...
	  !do fc = 1,num_soilc
		!c = filter_soilc(fc)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 't_scalar:', c, (t_scalar(c,j), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'w_scalar:', c, (w_scalar(c,j), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'o_scalar:', c, (o_scalar(c,j), j = 1, nlevdecomp)
		!write(iulog,"(A10,I4,15(1X,F10.4))") 'depth_sca:', c, (depth_scalar(c,j), j = 1, nlevdecomp)
	  !end do
