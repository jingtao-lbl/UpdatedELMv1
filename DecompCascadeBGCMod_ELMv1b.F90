            ...
            do j = 1, nlevdecomp
               do fc = 1,num_soilc
			     ...
			     if (TFun .le. 8) then
				  ...
				  Q10=1.9
				  Tref = 10._r8
				  else if (TFun .eq. 16) then 
				  Q10=2.0
				  Tref = 10._r8
				  else if (TFun .eq. 17) then 
				  Q10=2.1
				  Tref = 10._r8
				  else if (TFun .eq. 18) then 
				  Q10=2.2
				  Tref = 10._r8
				  else if (TFun .eq. 19) then 
				  Q10=2.3
				  Tref = 10._r8
				  else if (TFun .eq. 20) then 
				  Q10=2.4
				  Tref = 10._r8
				  else if (TFun .eq. 21) then 
				  Q10=2.5
				  Tref = 10._r8
				  else if (TFun .eq. 22) then 
				  Q10=1.3
				  Tref = 25._r8
				  else if (TFun .eq. 23) then 
				  Q10=1.4
				  Tref = 25._r8
				  else if (TFun .eq. 24) then 
				  Q10=1.5
				  Tref = 25._r8
				  else if (TFun .eq. 25) then 
				  Q10=1.6
				  Tref = 25._r8
				  else if (TFun .eq. 26) then 
				  Q10=1.7
				  Tref = 25._r8
				  else if (TFun .eq. 27) then 
				  Q10=1.8
				  Tref = 25._r8
				  else if (TFun .eq. 28) then 
				  Q10=1.9
				  Tref = 25._r8
				  else if (TFun .eq. 29) then 
				  Q10=2.0
				  Tref = 25._r8
				  else if (TFun .eq. 30) then 
				  Q10=2.1
				  Tref = 25._r8
				  else if (TFun .eq. 31) then 
				  Q10=2.2
				  Tref = 25._r8
				  else if (TFun .eq. 32) then 
				  Q10=2.3
				  Tref = 25._r8
				  else if (TFun .eq. 33) then 
				  Q10=2.4
				  Tref = 25._r8
				  else if (TFun .eq. 34) then 
				  Q10=2.5
				  Tref = 25._r8
				  else if (TFun .eq. 35) then 
				  Q10=1.5
				  Tref = 35._r8
				  else if (TFun .eq. 36) then 
				  Q10=2.0
				  Tref = 35._r8
				  else if (TFun .eq. 37) then 
				  Q10=2.5
				  Tref = 35._r8
				  end if
				  
				  t_scalar(c,j)= (Q10**((t_soisno(c,j)-(SHR_CONST_TKFRZ+Tref))/10._r8))
					...
					if (TFun .eq. 38) then 
					froz_q10 = 1.3
					else if (TFun .eq. 39) then 
					froz_q10 = 2.0
					else if (TFun .eq. 40) then 
					froz_q10 = 2.5
					end if
				  ...
				  t_scalar(c,j) = max(min(t_scalar(c,j), 1.0_r8), 0.01_r8)
         ...
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
			   ...
			   if (WFun .le. 10) then
			     ...
			     if (WFun .eq. 1) then
			     !W1: Standcarb, Harmon and Domingo [2001]
			     w_scalar(c,j) = ((1-exp(-1._r8*(3._r8/15._r8)*(vsm_pec*100+0._r8)))**5._r8)*(exp(-1._r8*(vsm_pec*100/(90._r8+4._r8))**15._r8))
			     use_fo = .false.	!Oxygen stress already built in Standcarb function
			     
			     else if (WFun .eq. 2) then
			     !W2: Daycent for coarse-textured soils, Kelly et al. [2000]
			     w_scalar(c,j) = ((vsm_pec-1.70)/(0.55-1.70))**(3.22*((1.70-0.55)/(0.55+0.007)))*((vsm_pec+0.007)/(0.55+0.007))**3.22
			     use_fo = .false.	!Oxygen stress already built in Daycent function
			     
			     else if (WFun .eq. 3) then
			     !W3: Skopp et al. [1990]
			     w_scalar(c,j) = min(2*vsm_pec**1.3,2*(1-vsm_pec)**0.8)
			     use_fo = .false.	!Oxygen stress already built in Skopp function
			     
			     else if (WFun .eq. 4) then
			     !W4: Moyano, Moyano et al. [2013]
			     w_scalar(c,j) = 3.11*vsm_pec-2.42*vsm_pec*vsm_pec
			     use_fo = .false.	!Oxygen stress already built in Moyano function
			     
			     else if (WFun .eq. 5) then
			     !W5: Century, Parton et al. [2001] and Adair et al. [2008]
			     w_scalar(c,j) = min(1.0_r8, 1/(1+30*exp(-8.5*vsm_pec)))
			     use_fo = .true.
			     
			     else if (WFun .eq. 6) then
			     !W6: Demeter, Foley [2011]
			     w_scalar(c,j) = 0.25+0.75*vsm_pec
			     use_fo = .true.
			     
			     else if (WFun .eq. 7) then
			     !W7: Candy, Bauer et al. [2008]
			     ...
			     else if (WFun .eq. 8) then
			     !W8: Gompertz, Janssens et al. [2003]
			     w_scalar(c,j) = exp(-1*exp(0.824-0.308*vsm_pec*100))
			     use_fo = .true.
			     
			     else if (WFun .eq. 9) then
			     !W9: Myers et al. [1982]
			     w_scalar(c,j) = 1.2*vsm_pec+(1-1.2)*vsm_pec**2	!Myers1
			     use_fo = .true.
			     
			     else if (WFun .eq. 10) then
			     !W10: Myers et al. [1982]
			     w_scalar(c,j) = 2.2*vsm_pec+(1-2.2)*vsm_pec**2	!Myers2
			     use_fo = .true.
			     
			     end if
			   
			   else if ((WFun .gt. 10).and.(WFun .le. 13)) then
			   
				 if (WFun .eq. 11) then
			     !W11: ELM Default
			     minpsi = -10.0_r8 !MPa
			     else if (WFun .eq. 12) then
			     !W12: ELM-S1 
			     minpsi = -1000.0_r8 !MPa
			     else if (WFun .eq. 13) then
			     !W13: ELM-S2
			     minpsi = -1000000.0_r8 !MPa
			     end if
			     ...
			     Sfop = 0.50
			     bp = 0.50
			     else if (WFun .eq. 15) then
			     !W15: Yan et al (2018)
			     Sfop = 0.65
			     bp = 0.50
			     else if (WFun .eq. 16) then
			     !W16: Yan et al (2018)
			     Sfop = 0.80
			     bp = 0.50
			     else if (WFun .eq. 17) then
			     !W17: Yan et al (2018)
			     Sfop = 0.50
			     bp = 0.75
			     else if (WFun .eq. 18) then
			     !W18: Yan et al (2018)
			     Sfop = 0.65
			     bp = 0.75
			     else if (WFun .eq. 19) then
			     !W19: Yan et al (2018)
			     Sfop = 0.80
			     bp = 0.75
			     else if (WFun .eq. 20) then
			     !W20: Yan et al (2018)
			     Sfop = 0.5
			     bp = 1.0
			     else if (WFun .eq. 21) then
			     !W21: Yan et al (2018)
			     Sfop = 0.65
			     bp = 1.0
			     else if (WFun .eq. 22) then
			     !W22: Yan et al (2018)
			     Sfop = 0.80
			     bp = 1.0
			     end if
				 
			     Yana = max(min(2.8*clayfrac(c,j)*0.01-0.046,1.0_r8),0.0_r8)
			     if (vsm_pec .ge. Sfop) then
			     	  w_scalar(c,j) = ((1-vsm_pec)/(1-Sfop))**bp
			     else
			     	  w_scalar(c,j) = ((0.1+Sfop*watsat(c,j))/(0.1+vsm_pec*watsat(c,j)))*(vsm_pec/Sfop)**(1.0_r8+2.0_r8*Yana)
			     end if
			     use_fo = .false.	!Oxygen stress already built in Yanetal function
			   
			   end if
			   w_scalar(c,j) = max(min(w_scalar(c,j), 1.0_r8), 0.01_r8)
			   !!---------Jing Tao -------
      ...
      if (use_vertsoilc) then
         do j = 1,nlevdecomp
            do fc = 1,num_soilc
			   ...
			   !Without Oxygen Scalar since oxygen stress already built in moisture functions
